...
if mode == 1:
    ...
    from keras.layers import LayerNormalization, MultiHeadAttention, Add, GlobalAveragePooling1D
    ...
    def transformer_encoder(x, head_size, num_heads, ff_dim, dropout):
        ...
        ff = Dense(int(x.shape[-1]))(ff)  # back to d_model
        x = Add()([x, ff])
        x = LayerNormalization(epsilon=1e-6)(x)
        return x
    
    # ===== Transformer hyperparameters (small + fair) =====
    d_model = 32
    head_size = 16
    num_heads = 2
    ff_dim = 64
    num_blocks = 2   # 2 encoder blocks
    
    inputs = Input(shape=(look_back, 1))
    
    # Project 1 feature -> d_model
    x = Dense(d_model)(inputs)
    
    # Positional embedding (learned)
    positions = tf.range(start=0, limit=look_back, delta=1)
    pos_embed = tf.keras.layers.Embedding(input_dim=look_back, output_dim=d_model)(positions)
    x = x + pos_embed
    ...
    num_layers = int(input("Enter number of layers/blocks used in this run: "))
    ...
    plt.title('Temperature Prediction (Transformer)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
